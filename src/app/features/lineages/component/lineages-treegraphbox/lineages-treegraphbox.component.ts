...
@Component({
  ...
  styleUrls: ['./lineages-treegraphbox.component.scss'],
})
export class LineagesTreegraphboxComponent implements OnInit {
  Highcharts = Highcharts;
  chartOptions: Highcharts.Options = {};

  ngOnInit(): void {
    this.chartOptions = {
      title: { text: 'Treegraph with box layout' },
      chart: { height: 600 },
      series: [{
        type: 'treegraph',
        data: [
        ...
        marker: { symbol:'rect', width:100 },
        // borderRadius:10,
        dataLabels: { enabled:true, formatter() { return this.point.name; }, style:{ whiteSpace:'nowrap' } },
        levels:[{ level:1 },{ level:2, colorVariation:{ key:'brightness', to:-0.5 } },{ level:3, colorVariation:{ key:'brightness', to:0.5 } }]
      }]
    };
  }
}
