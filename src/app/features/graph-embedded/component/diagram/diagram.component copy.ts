...
export class DiagramComponent implements AfterViewInit {
  ...
  //       const sourceName = sourceElement?.attributes?.attrs?.['title']?.text || 'Unnamed';
  //       console.log('📌 Source Element:', sourceName, sourceElement?.attributes);
  ...
  //         console.log(`🟢 Link from Source: ID=${l.id}, Source=${l.get('source')?.id}, Target=${l.get('target')?.id}`);
  //       });
  //     }

  //     console.log(`🔗 Link ID=${link.id}, Source=${sourceId}, Target=${link.get('target')?.id}`);
  //   });
  ...
  ngAfterViewInit() {
    ...
    container.addEventListener('drop', (e: DragEvent) => {
      ...
      const shapeType = block.type || 'SingleBlock';
      const ShapeClass = joint.shapes?.processor.RecordShape
        // shapeType === 'DualBlock'
        //   ? joint.shapes?.processor.DualBlock
        //   : joint.shapes?.processor.SingleBlock;


        const order = new ShapeClass({
        items: [[{
            id: 'file',
            label: 'File: (default)',
